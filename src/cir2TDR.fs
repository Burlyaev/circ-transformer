...
let cir2tdrF (cirD: cirDicT) voterL =    
    ...
    let oldIOL= List.append  (List.map (fun str->str+",") oldIOLwoCtr) ["\t input save,"; //inps+outs+ heart-beat control
                                                                        "\t input rollBack,"
                                                                        "\t output fail"]
    ...
    let reg2RegBlock (number:int) regName= 
        ...
        let result=
                match ifVot with 
                |true -> ("ff2B "+(reg2rBlockRenF regName)+"(clk, "+singleInp.ToString()+", "+ regName + ", save, rollBack, ErrDet["+number.ToString()+"]);" )//if FF-block with compare --[TODO]compare optim.
                |false ->("ff2B "+(reg2rBlockRenF regName)+"(clk, "+singleInp.ToString()+", "+ regName + ", save, rollBack, ErrDet["+number.ToString()+"]);") //iff FF-block w/o voter--[TODO]compare optim.
        result
   ...
   //process Definition- full strings list
    ...
    let topOutStrL=  (Array.toList outStrA) |> 
                                    List.map  (fun outName->"\t output[2:0] "+outName) //outputs declaration
    ...
    //generate wires
    let rollBackL= ["\t wire rollBack;"]           // heart-beat wire from Beat-Block to circuit
    let saveL=  ["\t wire save;"]      // control of the recovery procedure
    ...
    let ioRen oldName= "_N_"+oldName; // renaming function for input/output interface wires
    ...
    let instansRedModStr interfSeqStr=
        ...
        let interf = List.append  clkInterf ["save"; "rollBack";"fail"]
    ...
    let ctrBlockStrL= ["\t ctr2RecT heartBeat(clk, RESET_G, 
                                                failGL,save, rollBack,
                                                    readBuff,subst);"] // Heard-control block
    ...
    //TODO [TO Investigate]
    // recovery 2-> will be not visible- if the comparator said that error is detected during 2nd cycle
    // the we have dedicated output signal to the next DFF-block- we indicate to rewrite it's reg2 with it's reg1
    // and that's it-> recovery takes 0 cycles, we are in time. 
    //if we just before the primary output, then we ask ignore this signal and take the previous one as correct
    // if SEU occurs at this dedicated Err-signla wire:-- 1st heard beat: reg1->reg2 of
    // all forwarding blocks, but reg2=reg1 at this moment, so all is ok.
    //  --2nd heard beat: also ok, since it should be reg2 becoming reg1 anyway.
