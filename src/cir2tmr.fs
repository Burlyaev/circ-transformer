...
let cir2tmrF (cirD: cirDicT) voterL =    
   ...
   //process Definition
    ...
    //instantiate voters
    let votStrL=List.map3 (fun inp out name-> "\t voter3 "+"vot_"+name+"("+out+","+inp+");") voterL votOutNameL voterL

    let fileTopL= List.fold List.append [] [modFin;votOutWirL;memOutWirL;tmrStrL;votStrL;["endmodule"]]
    //
