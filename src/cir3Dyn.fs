...
let cir3dynF (cirD: cirDicT) voterL =    
    ...
    let reg2RegBlock (number:int) regName= 
        ...
        let result=
                match ifVot with 
                |true -> ("ff3DB "+(reg2rBlockRenF regName)+"(clk, "+singleInp.ToString()+", "+ regName + ", modeS, fetchA, ErrDet["+number.ToString()+"]);" )//if FF-block with compare --[TODO]compare optim.
                |false ->("ff3DB "+(reg2rBlockRenF regName)+"(clk, "+singleInp.ToString()+", "+ regName + ", modeS, fetchA, ErrDet["+number.ToString()+"]);") //iff FF-block w/o voter--[TODO]compare optim.
        result
   ...
   //process Definition- full strings list
    ...
    let modFin= List.append modTop [", input userMode, input userFetch, output userFail);"] //final TOP TMR module interface
    ...
    let assFGlob= ["\t assign failGL=fail;"]
    ...
    let instansRedModStr interfSeqStr=
        ...
        let interf = List.append  clkInterf ["modeS"; "fetchA";"fail"]
    ...
    let ctrBlockStrL= ["\t ctr3DynT heartBeat(clk, RESET_G, 
                                                failGL,modeS, fetchA,
                                                    userMode,userFetch,userFail);"] // Heard-control block
